...
- **ADR-061** Argos is offline-primary translator (Argos→DeepL→Google), run in a **single persistent subprocess** (native runtime segfaults co-resident with torch/faiss; multiple workers thrash). Translation metadata and output notes must name the provider(s) actually used, including mixed Argos+fallback documents.
...
**UI** — `UI/` package (Gradio), launched by the thin `app.py` entry point (`python app.py` → localhost:7860). One module per screen plus shared infrastructure: `UI/app.py` (assembles screens + cross-screen wiring + `launch()`), `utils.py` (paths, run/artifact discovery, pillar hints), `styles.py` (ALL CSS — page-level `APP_CSS` + inline `PIPELINE_CSS`/`REPORT_CSS`/`COST_CSS` sharing one theme-variable preamble), `header.py` (brand bar + light/dark toggle), `pipeline_view.py` (run-state machine `update_run_state()` + `render_pipeline()`), `reports.py` (`gather_report_data()` → `render_report_html()`/`render_report_markdown()`, per-run artifact persistence + backfill, `render_cost_report()`), and `run_screen.py` / `results_screen.py` / `configure_screen.py` / `environment_screen.py` (each exposes `build_*_screen()` returning its components for wiring).
Behaviour: the Run screen streams `main.py` as a subprocess into a three-phase pipeline visual (Zone 1 prep → a **loop frame** that resets per discovered document, with per-document chips and progress → finalise). On success the steps collapse and a document-style run report renders — **all counts come from the run's own final CSV/JSON, not any mid-run tally** — persisted as `<base>_runReport.md` + `<base>_cost.json` next to the CSV (`persist_run_artifacts()`; `backfill_all_runs()` backfills older runs with a labelled synthetic cost placeholder). The Run Cost tab is tenant-scoped and loads the authenticated user's newest per-run cost artifact; it must never initialize from the shared legacy cost report. "View Results →" jumps to Results with the run selected. Results shows per selected run: CSV/JSON, Round 1 comparison with mismatched rows highlighted via a pandas Styler (✗ = engine missed a Round 1 act, ⚠ = finding not in Round 1; "no barrier" placeholder rows count as a miss), the saved Run report, per-run Cost, Download. My Settings is tenant-scoped: saved provider/model/contact and encrypted API keys hydrate whenever the tab opens; keys are masked by default and only shown in plain text after the user enables the reveal control. Configure validates through `EconomyConfig`/`validate_taxonomy`; Environment is the single `.env` editor covering LLM provider/model/keys and every other var the code actually reads (blank fields never clobber, secrets never surface back — the former LLM settings tab was folded into it). Styling tracks light/dark via Gradio CSS variables; the header toggle drives `body.dark` + `colorScheme` from JS and persists in `localStorage` (Gradio scopes custom CSS under `.gradio-container`, so `body.dark` selectors in CSS would never match).
